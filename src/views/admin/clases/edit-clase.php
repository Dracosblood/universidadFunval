                    ...
                    <div class="flex flex-col justify-center items-center">
                        ...
                        <form action="/index.php" method="post" class="flex flex-col gap-3 w-1/2 max-w-xl bg-white p-10 shadow-xl rounded-md">
                            ...
                            <div class="flex flex-col">
                                ...
                                <input type="text" id="name" name="name" value="<?= $clase["name"] ?>" class="border-2 p-2 rounded-md">
                            </div>
                            ...
                            <select name="instructor_id" id="instructor" class="border-2 p-2 rounded-md">
                                <?php foreach ($instructors as $row) {
                                    $selected = ($row['id'] == $clase["instructor_id"]) ? "selected" : ""; ?>

                                    <option <?= $selected ?> value="<?= $row['id']; ?>"><?= $row['fullname']; ?></option>
                                <?php } ?>
                            </select>

                            <input type="hidden" name="action" value="edit-clase">

                            <div class="flex justify-end ">
                                <input type="submit" name="create-student" value="Editar Clase" class="bg-blue-500 text-white p-3 rounded-md cursor-pointer">
                            </div>
                        </form>
                    </div>
